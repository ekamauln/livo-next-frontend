...
import { MbOnlineForm } from "@/components/forms/mb-online-form";
...
export default function MbOnlinesTable() {
  ...
  // Fetch mb-onlines data
  const fetchData = useCallback(
    async (page: number = 1, search: string = "") => {
      try {
        setIsLoading(true);
        const response = await mbOnlineApi.getMbOnlines(
          page,
          pagination.limit,
          search
        );
        // Extract mb-onlines array from the response data
        const mbOnlines = response.data.mb_onlines as MbOnline[];
        setData(mbOnlines);
        ...
        if (error instanceof ApiError) {
          if (error.status >= 500 || error.status === 0) {
            console.error("Server/Network error fetching mb-onlines:", error);
          } else {
            console.debug(
              "Client error fetching mb-onlines:",
              error.message,
              "Status:",
              error.status
            );
          ...
          console.error("Unexpected error fetching mb-onlines:", error);
        }

        let errorMessage = "Failed to fetch mb-onlines. Please try again.";
  ...
  const handleMbOnlineCreated = () => {
    // Refresh the data after creating a new mb-online
  ...
  const columns: ColumnDef<MbOnline>[] = [
    {
      accessorKey: "id",
    ...
    <div className="w-full space-y-4">
      <div className="grid grid-cols-4 gap-4">
        <div className="col-span-2 flex flex-col border p-4 rounded-md">
          <h3 className="text-lg font-semibold mb-4">Create MB-Online</h3>
          <Separator className="mt-0 mb-6" />
          <MbOnlineForm onMbOnlineCreated={handleMbOnlineCreated} />
        </div>
        <div className="col-span-2 flex flex-col border p-4 rounded-md">
          <h3 className="text-lg font-semibold mb-4">MB-Online Status</h3>
          <Separator className="mt-0 mb-6" />
          <MbOnlineStatus totalRecords={pagination.total} target={3000} />
        </div>
      </div>
