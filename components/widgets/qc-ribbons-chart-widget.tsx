...
export function QcRibbonsChartWidget() {
  ...
  useEffect(() => {
    const fetchQcRibbonData = async () => {
      try {
        setIsLoading(true);

        const response = await qcRibbonApi.getQcRibbonChart();
        ...
        console.error("Error fetching QC Ribbon chart data:", error);
        ...
        toast.error("Failed to load QC Ribbon statistics");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>QC Ribbons - Current Month</CardTitle>
          <CardDescription>
            Daily QC Ribbon processing statistics
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="flex justify-center items-center min-h-[250px]">
          <p className="text-sm text-muted-foreground">
            No QC Ribbon data available for this month
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>QC Ribbons - Current Month</CardTitle>
        <CardDescription>
          Showing total QC Ribbons processed in {currentMonth}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex w-full items-start gap-2 text-sm">
          <div className="grid gap-2">
            ...
            <div className="text-muted-foreground flex items-center gap-2 leading-none">
              Total: {totalCount} QC Ribbons in {currentMonth}
            </div>
          </div>
        </div>
      </CardFooter>
