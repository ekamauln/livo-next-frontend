...
export function QcOnlinesChartWidget() {
  ...
  useEffect(() => {
    const fetchQcOnlineData = async () => {
      try {
        setIsLoading(true);

        const response = await qcOnlineApi.getQcOnlineChart();
        ...
        console.error("Error fetching QC Online chart data:", error);
        ...
        toast.error("Failed to load QC Online statistics");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>QC Onlines - Current Month</CardTitle>
          <CardDescription>
            Daily QC Online processing statistics
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="flex justify-center items-center min-h-[250px]">
          <p className="text-sm text-muted-foreground">
            No QC Online data available for this month
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>QC Onlines - Current Month</CardTitle>
        <CardDescription>
          Showing total QC Onlines processed in {currentMonth}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex w-full items-start gap-2 text-sm">
          <div className="grid gap-2">
            ...
            <div className="text-muted-foreground flex items-center gap-2 leading-none">
              Total: {totalCount} QC Onlines in {currentMonth}
            </div>
          </div>
        </div>
      </CardFooter>
