...
export function PcOnlinesChartWidget() {
  ...
  useEffect(() => {
    const fetchPcOnlineData = async () => {
      try {
        setIsLoading(true);

        const response = await pcOnlineApi.getPcOnlineChart();
        ...
        console.error("Error fetching PC Online chart data:", error);
        ...
        toast.error("Failed to load PC Online statistics");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>PC Onlines - Current Month</CardTitle>
          <CardDescription>
            Daily PC Online processing statistics
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="flex justify-center items-center min-h-[250px]">
          <p className="text-sm text-muted-foreground">
            No PC Online data available for this month
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>PC Onlines - Current Month</CardTitle>
        <CardDescription>
          Showing total PC Onlines processed in {currentMonth}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex w-full items-start gap-2 text-sm">
          <div className="grid gap-2">
            ...
            <div className="text-muted-foreground flex items-center gap-2 leading-none">
              Total: {totalCount} PC Onlines in {currentMonth}
            </div>
          </div>
        </div>
      </CardFooter>
