...
export function OutboundsChartWidget() {
  ...
  useEffect(() => {
    const fetchOutboundData = async () => {
      try {
        ...
        const response = await outboundApi.getOutboundChart();
        ...
        console.error("Error fetching Outbound chart data:", error);
        ...
        toast.error("Failed to load Outbound statistics");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>Outbounds - Current Month</CardTitle>
          <CardDescription>
            Daily Outbound processing statistics
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="flex justify-center items-center min-h-[250px]">
          <p className="text-sm text-muted-foreground">
            No Outbound data available for this month
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>Outbounds - Current Month</CardTitle>
        <CardDescription>
          Showing total Outbounds processed in {currentMonth}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex w-full items-start gap-2 text-sm">
          <div className="grid gap-2">
            ...
            <div className="text-muted-foreground flex items-center gap-2 leading-none">
              Total: {totalCount} Outbounds in {currentMonth}
            </div>
          </div>
        </div>
      </CardFooter>
