...
export function MbOnlinesChartWidget() {
  ...
  useEffect(() => {
    const fetchMbOnlineData = async () => {
      try {
        setIsLoading(true);

        const response = await mbOnlineApi.getMbOnlineChart();
        ...
        console.error("Error fetching MB Online chart data:", error);
        ...
        toast.error("Failed to load MB Online statistics");
      } finally {
        setIsLoading(false);
      }
    };
  ...
  if (isLoading) {
    return (
      <Card>
        <CardHeader>
          <CardTitle>MB Onlines - Current Month</CardTitle>
          <CardDescription>
            Daily MB Online processing statistics
          </CardDescription>
        </CardHeader>
      ...
      <Card>
        ...
        <CardContent className="flex justify-center items-center min-h-[250px]">
          <p className="text-sm text-muted-foreground">
            No MB Online data available for this month
          </p>
        </CardContent>
      </Card>
    );
  ...
  return (
    <Card>
      <CardHeader>
        <CardTitle>MB Onlines - Current Month</CardTitle>
        <CardDescription>
          Showing total MB Onlines processed in {currentMonth}
        </CardDescription>
      </CardHeader>
      <CardContent>
        ...
        <div className="flex w-full items-start gap-2 text-sm">
          <div className="grid gap-2">
            ...
            <div className="text-muted-foreground flex items-center gap-2 leading-none">
              Total: {totalCount} MB Onlines in {currentMonth}
            </div>
          </div>
        </div>
      </CardFooter>
